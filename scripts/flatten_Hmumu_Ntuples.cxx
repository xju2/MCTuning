// 
// Create a Ntuple that contians only BDT inputs
//
#include <TFile.h>
#include <TVector2.h>
...
int flatten(const char* in_file_name, const char* out_file_name, bool reweight=true)
{
	...
	oldtree->SetBranchStatus("McEventWeight", 1);
	oldtree->SetBranchStatus("ExpWeight", 1);
	...
	oldtree->SetBranchAddress("Event_MET", &event_MET);
	oldtree->SetBranchAddress("Muons_CosThetaStar", &costheta);

	TFile* out_file = TFile::Open(out_file_name, "RECREATE");
	TTree* newtree = new TTree(tree_name, "BDT Variables");
	if (lumi){
		lumi->Write();
	}
	...
	newtree->Branch("Event_MET", &met_, "Event_MET/F");
	newtree->Branch("Muons_CosThetaStar", &costheta_, "Muons_CosThetaStar/F");
